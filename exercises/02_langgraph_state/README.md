...
- OpenAI o 系など、API が `reasoning_content` を返すモデルではその値を `[思考]` に使う
- それ以外（OpenRouter の一般的なチャットモデルなど）は、システムプロンプトで促した `<thinking>...</thinking>` をパースして表示する

タグに従わない応答のときは `[思考]` は出ません。
